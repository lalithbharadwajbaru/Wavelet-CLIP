...
@DETECTOR.register_module(module_name="xclip_wavelet")
class XCLIPDetector(AbstractDetector):
    def __init__(self, config):
        ...
        # base 768 , large 1024
        self.fc_norm = nn.LayerNorm(1024)
        ...
        # Define the linear layer for processing DWT coefficients
        # 1024 --> 137, 768 -->
    ...
    def build_backbone(self, config):
        ...
        # xclip-base-patch32, xclip-large-patch14
        backbone = XCLIPVisionModel.from_pretrained("microsoft/xclip-large-patch14")
        return backbone

    def build_temporal_module(self, config):
        model = nn.LSTM(
            input_size=2048, hidden_size=512, num_layers=3, batch_first=True
        )
        return model
    ...
    def features(self, data_dict: dict) -> torch.tensor:
        ...
        with torch.no_grad():
            batch_size, num_frames, num_channels, height, width = data_dict[
                "image"
            ].shape
            ...
            sequence_output = outputs["pooler_output"].reshape(
    ...
    def classifier(self, features: torch.tensor) -> torch.tensor:
        # Assume the input has been appropriately reshaped to fit DWT if needed
